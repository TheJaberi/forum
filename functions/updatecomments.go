...
func UpdateComments() {
	if LoggedUser.Registered { // if the user is logged in the fact that he has liked or disliked the post is saved in all posts
		for i := 0; i < len(AllData.Postpage.Comments); i++ {
			var interaction int
			postData := DB.QueryRow("SELECT interaction from interaction_comments where comment_id = ?, post_id = ? AND user_id = ?", AllData.Postpage.Comments[i].Comment_id, AllData.Postpage.PostID, LoggedUser.Userid)
